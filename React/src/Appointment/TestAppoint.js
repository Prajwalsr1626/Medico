...
export default class ConsulyanyAppoint extends React.Component{
    ...
    componentDidMount(){
      ...
      fetch(`http://localhost:8080/Hospital/Accpetconrequesttest/${this.props.Hosdata}`)
   ...
   notvisited(obj,index)
   {
    ...
    var ob={
        ...
        date: obj.date,
        time:obj.time,
        viname:obj.testnames,
        statusMessage:"Not Visited"
         }
        var appoint=[...this.state.Appointmenttest]
 
         appoint.splice(index,1)
        
         fetch(`http://localhost:8080/Hospital/visisted`,{
             method : 'POST',
             headers:{
             ...
             if(data.status)
             {
                 ...
                 this.setState({Appointmenttest:appoint})
             }
             else{
               this.setState({regmsg:data.msg})
             }
   ...
   refresh(){
    ...
    render(){
                ...
                <div class="card-body">
                 ...
                 <div class="row register-form" >
                    ...
                    <label>Specialization</label>
                        <input ref={c=>this.testnames=c} type="text"  value={this.testnames} class="form-control" required disabled  />
                    </div>
                    <div class="form-group">
                    <label>Test</label>
                        <input ref={c=>this.file=c}type="file" accept="application/pd" class="form-control" required  />
                    </div>
                    <button onClick={()=>this.uplodefile()} class="btn btn-success"> Uplode</button>
                    </div>
        ...
        return(
            ...
            <div class="tab-pane fade" id="testAppoint" role="tabpanel" aria-labelledby="Consultancy-list">
